...
; Title:
;
;   PD09 -- PIC "4-pin" 10 MHz to 1PPS frequency divider (20 us pulse)
;
; Function:
;
...
;
;   The output is a narrow 20 microsecond pulse (not a 50% square wave).
;
; Diagram:
;                                ---__---
;                5V (Vdd)  +++++|1      8|=====  Ground (Vss)
;         10 MHz clock in  ---->|2  pD  7|---->  1PPS out (20 us)
;                              *|3  09  6|*
;                              o|4      5|o
;                                --------
; Notes:
...
; second. To make 1PPS: set output high for a time once per loop.

        ; Set output high for 20 us = 50 Tcy at 10 MHz.

rise:   movlw   0xFF            ; (      1) high output
        movwf   GPIO            ; (      1) W -> output pin(s)
        movlw   d'45'           ; (      1)
        call    DelayW1         ; (     45) delay (15 <= W <= 255)
        goto    fall            ; (      2)

        ; Set output low for 999.980 ms = 2,499,950 Tcy at 10 MHz.

fall:   movlw   0x00            ; (      1) low output
        movwf   GPIO            ; (      1) W -> output pin(s)
        movlw   d'249'          ; (      1)
        call    DelayW10k       ; (2490000) delay W*10000
        movlw   d'99'           ; (      1)
        call    DelayW100       ; (   9900) delay W*100
        movlw   d'43'           ; (      1)
        call    DelayW1         ; (     43) delay (15 <= W <= 255)
        goto    rise            ; (      2)
