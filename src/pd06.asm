...
; Title:
;
;   PD06 -- PIC "4-pin" 10^6 frequency divider (10 MHz to 10 Hz)
;
; Function:
;
;   This PIC program implements a digital frequency divider: the external
;   input clock is divided by a factor of 1 million (1e6). For example,
;   if the input clock is 10 MHz then the output clock will be 10 Hz.
;
; Diagram:
;                                ---__---
...
; A 125,000 instruction (400 ns each) loop takes exactly 50 milliseconds.
; Output pins are toggled once per loop creating a 10 Hz square wave.
